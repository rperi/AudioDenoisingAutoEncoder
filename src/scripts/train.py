...
def main(dict_dir, model_dir):
    ...
    list_chunks_input = [int(x.split('.')[0].split('_')[2]) for x in os.listdir(dictionary_dir + '/noisy/' + 'train/')]
    list_chunks_target = [int(x.split('.')[0].split('_')[2]) for x in os.listdir(dictionary_dir + '/clean/' + 'train/')]

    if not set(list_chunks_input) == set(list_chunks_target):
    ...
    num_chunks_per_load = 1 #SmallestFactor(len(list_chunks_input), max_load_size)
